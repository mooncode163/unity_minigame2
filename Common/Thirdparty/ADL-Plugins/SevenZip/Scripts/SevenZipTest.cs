...
public class SevenZipTest : MonoBehaviour{
    ...
    IEnumerator buff2buffTest() {
		...
		//(editor, standalone example) for iOS, Android use an apropriate method to get the bytes buffer from the file.
		#if (UNITY_STANDALONE_LINUX || UNITY_STANDALONE_WIN || UNITY_STANDALONE_OSX) && (!UNITY_IOS && !UNITY_ANDROID) || UNITY_EDITOR
        ...
        //Download a file.
        using (UnityEngine.Networking.UnityWebRequest w = UnityEngine.Networking.UnityWebRequest.Get("http://telias.free.fr/tests/google.jpg.lzma"))
        {
